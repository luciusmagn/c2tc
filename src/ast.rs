...
impl Ast
{
	...
	pub fn new(ast_ptr: *mut mpc_ast_t) -> Ast
	{
		...
		for node in
			unsafe { slice::from_raw_parts((*ast_ptr).children, (*ast_ptr).children_num as usize) }
		{
	...
	pub fn by_tag(&self, tag: &str) -> Option<Child>
	{
		...
		let index: i32 =
			if children.clone().any(|x| x.tag == tag)
	...
	pub fn by_contents(&self, contents: &str) -> Option<Child>
	{
		let children = self.children.clone().into_iter();
		let index: i32 =
			if children.clone().any(|x| x.contents == contents)
				{ children.count() as i32 }
			else { -1 };
